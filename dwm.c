...
enum { SchemeStatus, SchemeSelTag, SchemeNorTag, SchemeWinTag, SchemeNorTit, SchemeSelTit, SchemeLtsymb, SchemeNorm, SchemeSel }; /* color schemes */
	...
	if (ev->window == root) {
		...
		if (updategeom() || dirty) {
			...
			for (m = mons; m; m = m->next) {
				...
				/*XMoveResizeWindow(dpy, m->barwin, m->wx + sp, m->by + vp, m->ww -  2 * sp, bh);*//*PATCH REVERSE NECESSARY ? */
				resizebarwin(m);
			}
			focus(NULL);
			arrange(NULL);
...
drawbar(Monitor *m)
{
	...
	drw_setscheme(drw, scheme[SchemeLtsymb]);
	x = drw_text(drw, x, 0, w, bh, lrpad / 2, m->ltsymbol, 0);

	if ((w = m->ww - tw - stw - x) > bh) {
		if (m->sel) {
			drw_setscheme(drw, scheme[m == selmon ? SchemeSelTit : SchemeNorTit]);
			drw_text(drw, x, 0, w - 2 * sp, bh, lrpad / 2 + (m->sel->icon ? m->sel->icw + ICONSPACING : 0), m->sel->name, 0);
			if (m->sel->icon) drw_pic(drw, x + lrpad / 2, (bh - m->sel->ich) / 2, m->sel->icw, m->sel->ich, m->sel->icon);
			if (m->sel->isfloating)
				drw_rect(drw, x + boxs, boxs, boxw, boxw, m->sel->isfixed, 0);
		} else {
			drw_setscheme(drw, scheme[SchemeNorTit]);
...
togglebar(const Arg *arg)
{
	...
	updatebarpos(selmon);
	/*XMoveResizeWindow(dpy, selmon->barwin, selmon->wx + sp, selmon->by + vp, selmon->ww - 2 * sp, bh);*//* PATCH REVERSE ??? */
	resizebarwin(selmon);
	if (showsystray) {
		XWindowChanges wc;
		if (!selmon->showbar)
